    ...
    export default {
      ...
      methods: {
          AddRemoveFavorite (movie) {
            this.getMovieWithFavorite()

            if (movie.is_favorite !== null) {
            this.$axios.delete(`/api/favorites/${movie.id}`).then( res => {
                // will add for loader or succes modal
            }).catch(err => {
                console.log(err)
            })
            } else {
          ...
          getMovieWithFavorite () {
            let latest_movie = []
            if (this.$auth.user) {
                this.movies.forEach((movie, key) => {
                    movie.users.forEach((movie_user, key) => {
                        if (movie_user.id == this.$auth.user.id) {
